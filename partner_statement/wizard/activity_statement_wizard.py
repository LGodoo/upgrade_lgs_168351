    ...
    def _mail(self):
        ...
        if self.include_outstanding == True:
           ...
           #_logger.info("PARTNER OBJECT : %r", partner_obj)
            #ir_model_obj = self.env['ir.model.data']
            template_browse = self.get_object_reference('partner_statement', 'statements_email_template')[1]
            ...
            for record in self.env['res.partner'].browse(active_ids):
                ...
                #    record.id, ['subject', 'body_html', 'email_from', 'email_to', 'partner_to', 'email_cc', 'reply_to', 'scheduled_date'])  # it is to generate email for specific object record
                data = self._prepare_statement()
                ...
                pdf = self.env.ref('partner_statement.action_print_outstanding_statement').sudo()._render_qweb_pdf(
                    [record.id], data=data)
                pdf1 = self.env.ref('partner_statement.action_print_activity_statement').sudo()._render_qweb_pdf(
                    [record.id], data=data)
                ...
                #email_template_obj.email_to = record.email
                #email_template_obj.email_cc = emails
                #record.message_post(type="email", body= "Dear " + record.name + ", Please find attached your activity report. Regards", force_send=True, notif_layout=False, attachments=[att1,att2])
                email_template_obj.attachment_ids = [(4, att2.id)]
                ...
                #email_template_obj.attachment_ids = [(6,0, [att1.id, att2.id])]
                record.message_post_with_template(email_template_obj.id)
                email_template_obj.attachment_ids = [(3, att2.id)]
            ...
            ir_model_obj = self.env['ir.model.data']
            template_browse = \
            self.get_object_reference('partner_statement', 'activity_statement_email_template')[1]
            email_template_obj = self.env['mail.template'].browse(template_browse)
            partner_ids = []
            ...
            for record in self.env['res.partner'].browse(active_ids):
                ...
                _logger.info("dataupdate: %r", data)
                emails = []
                pdf = self.env.ref('partner_statement.action_print_activity_statement').sudo()._render_qweb_pdf(
                    [record.id], data=data)
                b64_pdf = base64.b64encode(pdf[0])
                ...
                #composer = self.env['mail.compose.message'].with_context(
                 #   active_id=record.id,
                  ...
                  #  default_template_id=email_template_obj.id,
                ...
                #record.message_post(type="email", body= "Dear " + record.name + ", Please find attached your activity report. Regards", force_send=True, notif_layout=False, attachments=att1)
                #email_template_obj.attachment_ids = [(4, att1.id)]
                #email_template_obj.send_mail(record.id)#, force_send=True, email_values=values)
                #email_template_obj.attachment_ids = None
